...
class DashboardController extends Controller
{
    ...
    public function all()
    {
        ...
        $data['today_price'] = $this->getService('Order', true)->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '>=', now()->toDateTimeString())->sum('total_price'); // 今日销售额
        $data['yesterday_price'] = $this->getService('Order', true)->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '<', now()->toDateTimeString())->where('pay_time', '>=', Carbon::yesterday()->toDateTimeString())->sum('total_price'); // 昨日销售额
        $data['week_price'] = $this->getService('Order', true)->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '>=', Carbon::now()->startOfWeek()->toDateTimeString())->sum('total_price'); // 这周销售额
        $data['last_week_price'] = $this->getService('Order', true)->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '<', Carbon::now()->startOfWeek()->toDateTimeString())->where('pay_time', '>=', Carbon::now()->subWeeks(1)->startOfWeek()->toDateTimeString())->sum('total_price'); // 上周销售额
        $data['month_price'] = $this->getService('Order', true)->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '>=', Carbon::now()->startOfMonth()->toDateTimeString())->sum('total_price'); // 这月销售额
